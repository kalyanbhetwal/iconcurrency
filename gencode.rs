...
mod checkpoint {
    ...
    #[inline(never)]
    pub fn checkpoint(c_type: bool, dp: stm32f3xx_hal_v2::pac::Peripherals) {
        let r0_value: u32;
        let r1_value: u32;
        let r2_value: u32;
        let r3_value: u32;
...
pub mod app {
    ...
    use cortex_m_semihosting::{debug, hprintln};
    use cortex_m::asm::{nop, self};
    use crate::checkpoint::{self, delete_all_pg, restore, checkpoint};
    /// User code end
    ///Shared resources
    struct Shared {
        a: u32,
    ...
    fn init(ctx: init::Context) -> (Shared, Local) {
        let device: stm32f3xx_hal_v2::pac::Peripherals = ctx.device;
        unsafe {
            asm!("mov r6, #12");
            asm!("mov r8, #9");
            asm!("mov r9, #6");
        }
        checkpoint(false, device);
